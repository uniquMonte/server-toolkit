...
setup_ssh_key() {
    ...
    # Ask for username
    read -p "Enter username for key configuration (default: root) (press Enter for default): " username
    ...
    if [ "$username" == "root" ]; then
        ...
        if ! id "$username" &>/dev/null; then
            ...
            read -p "Create this user? (y/N) (press Enter to skip): " create_user
    ...
    if [ -f "$authorized_keys" ] && [ -s "$authorized_keys" ]; then
        ...
        cat "$authorized_keys"
        echo ""
        read -p "Add new key? (y/N) (press Enter to skip): " add_new
        if [[ ! $add_new =~ ^[Yy]$ ]]; then
            return
        fi
    fi
    ...
    case $key_method in
        ...
        2)
            ...
            read -p "Confirm to generate new key pair? (y/N) (press Enter to cancel): " confirm
            ...
            echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            echo ""
            read -p "Private key saved? (y/N) (press Enter to skip): " saved
            if [[ $saved =~ ^[Yy]$ ]]; then
                rm -f "$key_file"
...
change_ssh_port() {
    ...
    if sshd -t; then
        ...
        if command -v ufw &> /dev/null; then
            read -p "Automatically open new port in UFW? (Y/n) (press Enter to confirm): " open_port
...
configure_timeout() {
    ...
    read -p "Client alive interval in seconds (default: 60) (press Enter for default): " client_alive_interval
    client_alive_interval=${client_alive_interval:-60}

    read -p "Maximum alive count (default: 3) (press Enter for default): " client_alive_count
...
full_security_setup() {
    ...
    read -p "Change SSH port? (Y/n) (press Enter to confirm): " change_port
    ...
    read -p "Configure SSH timeout? (Y/n) (press Enter to confirm): " config_timeout
